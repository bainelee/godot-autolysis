...
var simulate_stall: bool = false
var display_count: int = 0

func delay_display_frame() -> void:
	if simulate_stall:
		display_count += 1
		if display_count % 12 == 0:
			OS.delay_msec(45)
...
func measure(sprinting: bool, use_display_stall: bool = false) -> Dictionary:
	...
	simulate_stall = use_display_stall
	await frames(120)
...
func run_checks() -> void:
	process_frame.connect(delay_display_frame)
	world = Node3D.new()
	...
	process_frame.disconnect(delay_display_frame)
	world.queue_free()
	await frames(3)
	quit()
