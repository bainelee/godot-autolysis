...
const EVIDENCE_PATH: String = "res://docs/project-autolysis/00-discuss/道具系统/道具系统p1实施证据/checks/slot-rays.json"
...
func _cast_ray(origin: Vector3, target: Vector3) -> Dictionary:
...
func _test_slot_rays() -> void:
	...
	for scene: PackedScene in scenes:
		for index: int in range(shelf.get_slot_count()):
			...
			item.finish_pickup()
			await _frames(2)
	_check(ray_records.size() == 72, "两种原药各自三十六槽位形成七十二条真实射线记录")
...
func _test_ray_occlusion(shelf: AutolysisRawMaterialShelf) -> void:
	...
	_check(back_result.get("collider", null) == shelf, "架体真实背板是从架后瞄准原药时的首个命中物")
	...
	_check(board_result.get("collider", null) == shelf, "架体下层板保持真实遮挡而不能从下方穿透取药")
...
func _save_ray_evidence() -> void:
	var absolute_path: String = ProjectSettings.globalize_path(EVIDENCE_PATH)
