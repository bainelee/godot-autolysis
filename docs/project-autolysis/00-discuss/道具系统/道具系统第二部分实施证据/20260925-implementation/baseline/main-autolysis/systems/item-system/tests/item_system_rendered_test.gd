...
const EVIDENCE: String = "res://docs/project-autolysis/00-discuss/道具系统/道具系统p1实施证据/checks"
...
func _test_main_scene() -> void:
	...
	for shelf_name: String in ["place_shelf_workroom_rm_0", "place_shelf_workroom_rm_1"]:
		...
		if item != null:
			...
			await _click()
			check(inventory.get_focused_item() != null and actual_shelf.get_item_at_slot(0) == null, "实际主场景架子指定瓶体取回：" + shelf_name)
